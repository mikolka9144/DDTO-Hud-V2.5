...
function onCreate()
	...
	local mirrorMode = getDataFromSave('DdtoV2', "mirror","none")
	local isNewPsych = version:find('0.7')
	if isNewPsych and mirrorMode == "complex" then
		enabled = true
		--luaDebugMode = true;
		addHaxeLibrary('Reflect')
		
		runHaxeCode(getTextFromFile('scripts/modules/HaxeFunctions.hx'), _, 'gimmeMyCallback')
